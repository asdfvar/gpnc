...
namespace comm {
         ...
         int       requested)
   {
      ...
      int maxRanks = 0;
      for (int stage = 0; stage < numStages; stage++)
         if (numStageProcs[stage] > maxRanks)
            maxRanks = numStageProcs[stage];
      ...
      for (int toStage = 0; toStage < numStages; toStage++)
      {
         ...
         // the communicator is created from the unioned group
         // between this stage and the associated stage in ascending numerical order
         // hence the starting rank
         // must be relative to where the associated stage starting rank will be
         // from this stage
         int startRank;
         ...
         MPI_Intercomm_create (
               *stageComms[thisStageNum],
               0,
               *unionStagesComms[lowStage][highStage - lowStage - 1],
               startRank,
               INTERCOMM_TAG,
               interComms[toStage]);
      }
      ...
      while (!unionStagesComms.empty()) {
         while (!unionStagesComms.back().empty()) {
            if (*unionStagesComms.back().back() != MPI_COMM_NULL)
               MPI_Comm_free (unionStagesComms.back().back());
   ...
   template bool COMM::send_to_stage (
         float       *data,
         int          dataSize,
         int          recvStage,
         unsigned int recvStageRank,
         ...
         int tag)
   {
      ...
      if (!found) {
         std::cout       << "send to module "    <<
            recvStage    << " from module "      <<
            thisStageNum << " request not found" <<
            std::endl;
         return false;
      }
   ...
   template bool COMM::receive_from_stage (
         float       *data,
         int          dataSize,
         int          sendStage,
         unsigned int sendStageRank,
         ...
         int tag)
   {
      ...
      // perform the blocking wait
      stat = MPI_Wait (
            receiveFromStageRequests[sendStage][tagIndex][sendStageRank],
            MPI_STATUS_IGNORE);
